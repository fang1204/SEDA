...
class CoPredictor(nn.Module):
    def __init__(self, cls_num, hid_size, biaffine_size, channels, ffnn_hid_size, dropout=0):
        ...
        # self.biaffine = Biaffine(n_in=biaffine_size, n_out=channels, bias_x=True, bias_y=True)
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.key_span_embed = nn.Embedding(3, config.lstm_hid_size,padding_idx=0)

    def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, span_ids, sent_length):
        '''
        ...
        # span_id => 00000-1-1-1
        # span_id => 11111000
        # entity_mask = (span_ids!=-1)
        # info_distribution = entity_mask * (span_ids + 1)
        # key_span = self.key_span_embed(info_distribution)
        # word_reps = word_reps+key_span
